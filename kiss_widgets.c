...
int kiss_pane_new(SDL_Surface *Surf, kiss_window *wdw, int decorate,
	int x, int y, int w, int h)
{
	if (!Surf) return -1;
	kiss_makerect(&Surf->clip_rect, x, y, w, h);
	//Surf->decorate = decorate;
	//Surf->visible = 1;
	//Surf->focus = 1;
	//Surf->wdw = wdw;
	return 0;
...
int kiss_pane_draw(SDL_Surface *Surf, SDL_Renderer *renderer)
{
	//if (Surf && window->wdw) window->visible = window->wdw->visible;
	//if (!window || !window->visible || !renderer) return 0;
	kiss_fillrectRenderer(renderer, &Surf->clip_rect);
	return 1;
}

int kiss_pane_event(kiss_window *window, SDL_Window *mWindow, SDL_Event *event, int *draw)
{
...
int kiss_window_event(kiss_window *window, SDL_Window *mWindow, SDL_Event *event, int *draw)
{
	if (!window || !window->visible || !event) return 0;
...
int kiss_window_draw(kiss_window *window, SDL_Renderer *renderer)
{
	...
	if (window->decorate)
		kiss_decorate(renderer, &window->rect, kiss_gray2, kiss_edge);
	return 1;
}
	...
	int x, int y, int w)
{
    ...
    if (hscrollbar->sliderLine.magic != KISS_MAGIC)
		hscrollbar->sliderLine = kiss_sliderLine;
...
int kiss_hscrollbar_draw(kiss_hscrollbar *hscrollbar, SDL_Renderer *renderer)
{
	...
	hscrollbar->leftrect.w = 0;
	hscrollbar->rightrect.w = 0;
		hscrollbar->sliderrect.x = hscrollbar->leftrect.x +
		hscrollbar->leftrect.w + (int) (hscrollbar->fraction *
		hscrollbar->maxpos) + 5;
    hscrollbar->sliderrect.y = hscrollbar->leftrect.y-3;
...
//		hscrollbar->rightrect.y, NULL);
    SDL_Rect srcrect;
    srcrect.w = hscrollbar->rightrect.x - hscrollbar->leftrect.x;
    srcrect.h = 16;
    srcrect.x = 4;
    srcrect.y = 0;
    kiss_renderimage(renderer, hscrollbar->sliderLine,hscrollbar->leftrect.x, hscrollbar->leftrect.y, &srcrect);
	...
	kiss_array *a, int x, int y, int w, int h)
{
	...
	textbox->bg = kiss_gray1;
	textbox->textcolor = kiss_white;
	textbox->hlcolor = kiss_gray2;
...
int kiss_textbox_draw(kiss_textbox *textbox, SDL_Renderer *renderer)
{
	...
	if (textbox->decorate)
		kiss_decorate(renderer, &textbox->rect, kiss_gray2,
			kiss_edge);
